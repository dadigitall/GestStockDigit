<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex gap-3">
            ...
            <select wire:model.live="filterStatus" class="border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm">
                ...
                @foreach(['draft', 'confirmed', 'preparing', 'ready', 'delivered', 'invoiced', 'paid', 'cancelled'] as $s)
    ...
    @if($showForm)
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 mb-6">
            <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-4">{{ $editId ? 'Modifier' : 'Nouvelle' }} commande client</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Date de commande <span class="text-red-500">*</span></label>
                    <input wire:model="orderDate" type="date" class="w-full border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm">
                    @error('orderDate') <span class="text-xs text-red-500 mt-1">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Date de livraison prévue</label>
                    <input wire:model="expectedDeliveryDate" type="date" class="w-full border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm">
                    @error('expectedDeliveryDate') <span class="text-xs text-red-500 mt-1">{{ $message }}</span> @enderror
                </div>
            </div>
            ...
            <div class="mt-4 flex gap-2">
                <button wire:click="save" class="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700">{{ $editId ? 'Mettre à jour' : 'Créer la commande' }}</button>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-5xl w-full mx-4 max-h-[90vh] flex flex-col">
                ...
                <div class="overflow-y-auto p-6 space-y-4">
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        ...
                        <div>
                            <span class="text-gray-500 text-xs">Date commande</span>
                            <p class="font-medium text-gray-900 dark:text-white">{{ $detail->order_date?->format('d/m/Y') ?? '—' }}</p>
                        </div>
                        <div>
                            <span class="text-gray-500 text-xs">Livraison prévue</span>
                            <p class="font-medium text-gray-900 dark:text-white">{{ $detail->expected_delivery_date?->format('d/m/Y') ?? '—' }}</p>
                        </div>
                        <div>
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                                    ...
                                    <th class="px-3 py-2 text-right font-medium text-gray-500 dark:text-gray-400 text-xs">Préparée</th>
                                    <th class="px-3 py-2 text-right font-medium text-gray-500 dark:text-gray-400 text-xs">Livrée</th>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach($detail->items as $item)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                        ...
                                        <td class="px-3 py-2 text-right text-gray-600 dark:text-gray-400 text-xs">{{ number_format($item->quantity_prepared, 0, ',', ' ') }}</td>
                            ...
                            <tfoot class="border-t border-gray-200 dark:border-gray-700">
                                <tr>
                                    <td colspan="7" class="px-3 py-2 text-right text-xs text-gray-600 dark:text-gray-400">Sous-total</td>
                                    <td class="px-3 py-2 text-right text-xs text-gray-900 dark:text-white font-medium">{{ number_format($detail->subtotal, 0, ',', ' ') }} F</td>
                                </tr>
                                <tr>
                                    <td colspan="7" class="px-3 py-2 text-right text-xs text-gray-600 dark:text-gray-400">TVA</td>
                                ...
                                @if($detail->discount > 0)
                                    <tr>
                                        <td colspan="7" class="px-3 py-2 text-right text-xs text-gray-600 dark:text-gray-400">Remise</td>
                                        <td class="px-3 py-2 text-right text-xs text-red-600 font-medium">-{{ number_format($detail->discount, 0, ',', ' ') }} F</td>
                                    </tr>
                                @endif
                                <tr class="border-t border-gray-200 dark:border-gray-700">
                                    <td colspan="7" class="px-3 py-2 text-right text-sm font-semibold text-gray-900 dark:text-white">Total</td>
                    ...
                    <div class="flex gap-2 pt-2">
                        @if($detail->status === 'draft')
                            <button wire:click="confirm({{ $detail->id }})" class="px-3 py-1.5 bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 text-xs font-medium rounded-lg hover:bg-blue-200 dark:hover:bg-blue-800/50">Confirmer</button>
                        @endif
                        @if($detail->status === 'confirmed')
                            <button wire:click="startPreparing({{ $detail->id }})" class="px-3 py-1.5 bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300 text-xs font-medium rounded-lg hover:bg-amber-200 dark:hover:bg-amber-800/50">Commencer la préparation</button>
                        @endif
                        @if($detail->status === 'preparing')
                            <button wire:click="markReady({{ $detail->id }})" class="px-3 py-1.5 bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 text-xs font-medium rounded-lg hover:bg-emerald-200 dark:hover:bg-emerald-800/50">Marquer prête</button>
                        @endif
                        @if($detail->status === 'ready')
                            <button wire:click="markDelivered({{ $detail->id }})" class="px-3 py-1.5 bg-teal-100 dark:bg-teal-900/50 text-teal-700 dark:text-teal-300 text-xs font-medium rounded-lg hover:bg-teal-200 dark:hover:bg-teal-800/50">Marquer livrée</button>
                            <button wire:click="markInvoiced({{ $detail->id }})" class="px-3 py-1.5 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 text-xs font-medium rounded-lg hover:bg-indigo-200 dark:hover:bg-indigo-800/50">Marquer facturée</button>
                        @endif
                        @if($detail->status === 'delivered')
                            <button wire:click="markInvoiced({{ $detail->id }})" class="px-3 py-1.5 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 text-xs font-medium rounded-lg hover:bg-indigo-200 dark:hover:bg-indigo-800/50">Marquer facturée</button>
                        @endif
                        @if($detail->status === 'invoiced')
                            <button wire:click="markPaid({{ $detail->id }})" class="px-3 py-1.5 bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300 text-xs font-medium rounded-lg hover:bg-green-200 dark:hover:bg-green-800/50">Marquer payée</button>
                        @endif
                        @if(in_array($detail->status, ['draft', 'confirmed', 'preparing']))
                            <button wire:click="cancel({{ $detail->id }})" onclick="return confirm('Annuler cette commande ?')" class="px-3 py-1.5 bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300 text-xs font-medium rounded-lg hover:bg-red-200 dark:hover:bg-red-800/50">Annuler</button>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($orders as $o)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                            <td class="px-4 py-3 text-xs font-mono font-medium text-gray-900 dark:text-white">{{ $o->reference }}</td>
                            <td class="px-4 py-3 text-xs text-gray-700 dark:text-gray-300">{{ $o->customer?->name ?? '—' }}</td>
                            <td class="px-4 py-3 text-xs text-gray-500">{{ $o->order_date?->format('d/m/Y') ?? $o->created_at?->format('d/m/Y') }}</td>
                            <td class="px-4 py-3 text-xs text-right font-medium text-gray-900 dark:text-white">{{ number_format($o->total, 0, ',', ' ') }} F</td>
                            <td class="px-4 py-3 text-center">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium {{ $this->statusBadge($o->status) }}">{{ $this->statusLabel($o->status) }}</span>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-1 flex-wrap">
                                    <button wire:click="view({{ $o->id }})" class="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-[10px] font-medium rounded hover:bg-gray-200 dark:hover:bg-gray-600">Détail</button>

                                    @if($o->status === 'draft')
                                        <button wire:click="edit({{ $o->id }})" class="px-2 py-1 bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 text-[10px] font-medium rounded hover:bg-blue-200 dark:hover:bg-blue-800/50">Modifier</button>
                                        <button wire:click="confirm({{ $o->id }})" class="px-2 py-1 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 text-[10px] font-medium rounded hover:bg-indigo-200 dark:hover:bg-indigo-800/50">Confirmer</button>
                                        <button wire:click="cancel({{ $o->id }})" onclick="return confirm('Annuler cette commande ?')" class="px-2 py-1 bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300 text-[10px] font-medium rounded hover:bg-red-200 dark:hover:bg-red-800/50">Annuler</button>
                                    @endif

                                    @if($o->status === 'confirmed')
                                        <button wire:click="startPreparing({{ $o->id }})" class="px-2 py-1 bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300 text-[10px] font-medium rounded hover:bg-amber-200 dark:hover:bg-amber-800/50">Préparer</button>
                                    @endif

                                    @if($o->status === 'preparing')
                                        <button wire:click="markReady({{ $o->id }})" class="px-2 py-1 bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 text-[10px] font-medium rounded hover:bg-emerald-200 dark:hover:bg-emerald-800/50">Prête</button>
                                    @endif

                                    @if($o->status === 'ready')
                                        <button wire:click="markDelivered({{ $o->id }})" class="px-2 py-1 bg-teal-100 dark:bg-teal-900/50 text-teal-700 dark:text-teal-300 text-[10px] font-medium rounded hover:bg-teal-200 dark:hover:bg-teal-800/50">Livrée</button>
                                        <button wire:click="markInvoiced({{ $o->id }})" class="px-2 py-1 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 text-[10px] font-medium rounded hover:bg-indigo-200 dark:hover:bg-indigo-800/50">Facturée</button>
                                    @endif
                                    ...
                                    @if($o->status === 'invoiced')
                                        <button wire:click="markPaid({{ $o->id }})" class="px-2 py-1 bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300 text-[10px] font-medium rounded hover:bg-green-200 dark:hover:bg-green-800/50">Payée</button>
                                    @endif

                                    @if(in_array($o->status, ['draft', 'confirmed', 'preparing']))
                                        <button wire:click="cancel({{ $o->id }})" onclick="return confirm('Annuler cette commande ?')" class="px-2 py-1 bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300 text-[10px] font-medium rounded hover:bg-red-200 dark:hover:bg-red-800/50">Annuler</button>
                                    @endif
                                </div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-400">Aucune commande client trouvée.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
