<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        @forelse($stores as $store)
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-white/5 p-5 hover:shadow-sm transition {{ $store->is_active ? '' : 'opacity-60' }}">
                <div class="flex items-start gap-3">
                    <div class="w-10 h-10 rounded-xl bg-gradient-to-br {{ match($store->type) {
                ...
                @if($store->parent)
                    <p class="text-xs text-slate-400 mt-2">
                        Rattachement : <span class="font-medium text-slate-600 dark:text-slate-400">{{ $store->parent->name }}</span>
                    </p>
                @endif
