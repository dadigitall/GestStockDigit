    ...
    @if($saleCompleted && $lastSale)
        ...
        <div class="max-w-md mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8">
            <div class="text-center mb-6">
                ...
                <h2 class="text-xl font-bold text-gray-900 dark:text-white">Vente en gros validée</h2>
            ...
            @if($lastSale->customer)
                <div class="text-sm text-center text-gray-600 dark:text-gray-400 mb-4">
                    Client: <strong>{{ $lastSale->customer->name }}</strong>
                </div>
            @endif
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 mt-4 pt-4 space-y-1 text-sm">
                ...
                @endif
                @if($lastSale->expected_delivery_date)
                    <div class="flex justify-between text-gray-500">
                        <span>Livraison prévue</span>
                        <span>{{ $lastSale->expected_delivery_date }}</span>
                    </div>
                @endif
            </div>

            <button wire:click="newSale" class="mt-4 w-full py-3 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 transition-colors">
                Nouvelle vente en gros
            </button>
        </div>
    ...
    @else
        ...
        <div class="flex flex-col lg:flex-row gap-6" style="min-height: calc(100vh - 12rem);">
            ...
            <div class="flex-1 flex flex-col">
                <div class="mb-2">
                    <input wire:model.live="barcode" wire:keyup="updatedBarcode" type="text" placeholder="Scanner un code-barres..." class="w-full px-4 py-2.5 border-2 border-emerald-400 dark:border-emerald-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-lg focus:ring-2 focus:ring-emerald-500 font-mono tracking-wider" autofocus>
                </div>
                <div class="mb-4">
                    <input wire:model.live.debounce.200ms="search" type="text" placeholder="Rechercher un produit (nom, réf., code-barres)..." class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-lg focus:ring-2 focus:ring-emerald-500">
                </div>
                <div class="flex-1 overflow-y-auto">
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3">
                        @foreach($products as $product)
                            <button wire:click="addToCart({{ $product->id }})" class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 hover:border-emerald-500 hover:shadow-md transition-all text-left {{ $product->is_stockable && $product->stock_quantity <= 0 ? 'opacity-50 cursor-not-allowed' : '' }}" {{ $product->is_stockable && $product->stock_quantity <= 0 ? 'disabled' : '' }}>
                                ...
                                @endif
                                <p class="text-lg font-bold text-emerald-600 dark:text-emerald-400 mt-2">{{ number_format($product->sale_price, 0, ',', ' ') }} F</p>
                                @if($product->wholesale_price)
                                    <p class="text-xs text-emerald-500 font-medium">Gros: {{ number_format($product->wholesale_price, 0, ',', ' ') }} F</p>
                                @endif
            ...
            <div class="lg:w-96 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 flex flex-col">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Vente en gros</h3>
                    <span class="text-sm text-gray-500">{{ count($this->cart) }} article(s)</span>
                </div>

                <!-- Customer selection (professional/reseller/wholesaler only) -->
                <div class="mb-4">
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Client professionnel</label>
                    <select wire:model.live="customerId" class="w-full text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2">
                        ...
                        @foreach($customers as $customer)
                            <option value="{{ $customer->id }}" title="{{ $customer->type_label ?? $customer->type }}">
                                {{ $customer->name }} ({{ $customer->type_label ?? $customer->type }})
                                @if($customer->balance > 0) - Solde: {{ number_format($customer->balance, 0, ',', ' ') }} F @endif
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Livraison prévue</label>
                    <input wire:model="expectedDeliveryDate" type="date" class="w-full text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2">
                </div>
                ...
                <div class="flex-1 overflow-y-auto space-y-2 mb-4">
                    @forelse($this->cart as $index => $item)
                        <div class="flex items-center gap-2 p-2.5 bg-gray-50 dark:bg-gray-700/30 rounded-lg">
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-500">{{ number_format($item['price'], 0, ',', ' ') }} F / unité</p>
                            </div>
                    ...
                    @empty
                        <div class="text-center py-12 text-gray-400">
                            ...
                            <p class="text-xs mt-1">Sélectionnez un client et des produits</p>
                        </div>
                    @endforelse
                </div>
                ...
                <!-- Commercial terms -->
                <div class="mb-3">
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Conditions commerciales</label>
                    <textarea wire:model="commercialTerms" rows="2" class="w-full text-xs border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-2 py-1.5" placeholder="{{ $customerId ? 'Conditions du client pré-remplies' : 'Optionnel' }}"></textarea>
                </div>
                ...
                <div class="border-t border-gray-200 dark:border-gray-700 pt-4 space-y-2">
                    ...
                    <button wire:click="openPayment" class="w-full py-3 bg-emerald-600 text-white font-medium rounded-lg hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors mt-2" {{ empty($this->cart) ? 'disabled' : '' }}>
        ...
        @if($showPaymentModal)
            <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" wire:key="payment-modal">
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6">Finaliser la vente en gros</h3>
                    ...
                    <div class="space-y-4">
                        ...
                        @if($paymentMethod === 'gift_card')
                            <div>
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <button wire:click="lookupGiftCard" class="px-4 py-2.5 bg-emerald-600 text-white text-sm rounded-lg hover:bg-emerald-700">Vérifier</button>
                                </div>
                                @if($giftCard)
                        ...
                        <div>
                            ...
                            <input wire:model.live="paidAmount" type="number" step="1" class="w-full text-2xl font-bold border-2 border-emerald-300 dark:border-emerald-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-4 py-3 focus:ring-2 focus:ring-emerald-500">
                        ...
                        <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
                            @if(!$secondaryPaymentMethod)
                                <button wire:click="$set('secondaryPaymentMethod', 'cash')" type="button" class="text-sm text-emerald-600 hover:text-emerald-700 font-medium">
                    ...
                    <div class="flex gap-3 mt-6">
                        ...
                        <button wire:click="confirmSale" class="flex-1 py-3 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors" {{ $totalPaid <= 0 && $paidAmount <= 0 ? 'disabled' : '' }}>
                            Confirmer la vente
                        </button>
                    </div>
                </div>
